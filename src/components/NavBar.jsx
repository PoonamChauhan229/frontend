...
const NavBar = () => {
  ...
  const { setShowSearch, wishlistItems, getWishlistCount } = useContext(ShopContext);  // Updated to get wishlist count
    ...
    <div className='flex items-center justify-between py-5 font-medium'>
      ...
      <div className='flex items-center gap-6'>
        ...
        <Link to='/wishlist' className='relative'>
          <FaRegHeart className='w-7 min-w-7' alt="Wishlist" style={{color: 'grey', fontSize: '230%'}} />
          <p className='absolute right-[-2px] bottom-[2px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]'>
            {getWishlistCount()}  {/* Display updated wishlist count */}
          </p>
        </Link>
        ...
        <Link to='/cart' className='relative'>
          ...
          <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]'>
            {getWishlistCount()}  {/* Display cart count, use separate function if needed */}
          </p>
        </Link>
